...
func (u User) List(limit, offset int) ([]User, error) {
	if limit < 0 { 
		return []User{}, errors.New("limit должно быть больше 0") 
	} 
	if offset < 0 { 
		return []User{}, errors.New("offset	должно быть больше 0") 
	} 
	if limit == 0 {
		return []User{}, nil
	}
...
func UserList(limit, offset int) ([]User, error) { 
		u := User{} 
		res, err := u.List(limit, offset) 
		if err != nil { 
			return res, errors.New("что-то пошло не так") 
		} 
		return res, nil 
} 
